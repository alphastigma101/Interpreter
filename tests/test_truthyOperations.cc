...
namespace Test {
class OperationsTest : public ::testing::Test, public TruthyOperations::truthyOperations {
    public:
        template<typename T>
        inline static void runTruthyTest(const char* testName, const Any value, bool expectedResult) {
...
TEST_F(OperationsTest, BinaryIDs) {
    {
        Vector<int> vec;
        Any value(vec);
        runTruthyTest<int>("Vector<int> test", value, true);
    }
    ...
    // Test Case 2: Pointer test
    {
        int* ptr = nullptr;
        Any value(ptr);
        runTruthyTest<int>("int* pointer test", value, true);
    }
    ...
    // Test Case 3: NukeInstance test
    {
        ...
        Any value(instance);
        runTruthyTest<int>("NukeInstance test", value, true);
    }
